...
class SubscriptionBackend extends GetxController {
  ...
  String calories = '500';
  Map dateMealList = {};
  CalendarModel calendarModel = dummyCalendarModel;
  ...
  addCalendarMeal() async {
    try {
      ...
      dateMealList.keys.forEach((element) async {
        ...
        var restauResponse = await HttpServices.get(
            "${ApiConstants.restaurantAmount}${value["restaurant"]}/${value['meals']}");

        if (restauResponse.statusCode == 200) {
          var resAmount = jsonDecode(restauResponse.body);
          Map restTranxBody = {
            "restaurant": value["restaurant"],
            "amount": resAmount['info']['price'],
            "debit": false,
            "description": "Order placed",
          };
          var trans = await HttpServices.post(
              ApiConstants.restaurantTransactionCreate,
              jsonEncode(restTranxBody));
          log("Create Trans: ${trans.body}");
        }
  ...
  addMealNote(DateTime dateTime, int index, String n) {
    try {
      String date = DateFormat('dd-MMM-yyyy').format(dateTime);
      DateFormat format = DateFormat('dd/MM/yyyy');
      List data = calendarModel.calendar
          ...
          .toList();
      if (dateMealList.containsKey(date)) {
        List note = dateMealList[date]['note'];
        note.length > index ? note[index] = n : note.add(n);
        dateMealList[date]['note'] = note;
      } else {
        Map data2 = {
          "order": data[0]['order'],
          "freezed": false,
          ...
          "carbs": data[0]['carbs'],
          "fat": data[0]['fat'],
          "protien": data[0]['protien'],
          "note": data[0]['note'],
        };
        dateMealList[date] = data2;
        dateMealList[date]['note'] = data[0]['note'];
        List note = dateMealList[date]['note'];
        note.length > index ? note[index] = n : note.add(n);
        dateMealList[date]['note'] = note;
      }
      log(dateMealList[date]['note'].toString());
      updateCalendar();
    } catch (e) {
      print("Add Note:$e");
    }
  ...
  fetchCalendarData(String orderID) async {
    try {
      String token = await StorageService().read(DbKeys.authToken);
      var response = await HttpServices.getWithToken(
          '${ApiConstants.fetchOrder}/$orderID', token);
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
  ...
  Future<bool> autoCalendarFillApi() async {
    try {
      ...
      while (i < duration) {
        log("while Indexing: $i");
        for (var items in subscriptionModel.restaurant) {
          var item = items.restaurant;
          ...
          await menuController.fetchRestaurantMenu(item.id).then((value) {
            if (menuController.restaurantModel!.menu.isNotEmpty) {
              if (subscriptionModel.meal.meals
                  .where((element) => element['label'] == 'Breakfast')
                  .toList()
                  .isNotEmpty) {
              ...
              if (subscriptionModel.meal.meals
                  .where((element) => element['label'] == 'Main')
                  .toList()
                  .isNotEmpty) {
                List bList = menuController.restaurantModel!.menu
                    .where((e) => e['category'] == 'main-dish')
                    .toList();
                log(bList.toString());
                addMeal(
                    dateTime,
              ...
              if (subscriptionModel.meal.meals
                  .where((element) => element['label'] == 'Snack/Soup')
                  .toList()
                  .isNotEmpty) {
                List bList = menuController.restaurantModel!.menu
          ...
          log("Indexing: $i");
          log("Indexing Date: $dateTime");
        }
      }
      if (i >= duration) {
        log(dateMealList.toString());
